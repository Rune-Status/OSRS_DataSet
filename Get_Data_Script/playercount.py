...
def parse_data(resp,d):
  ...
  if jsonfile['osrs'][-1][0]-15*60*1000 not in dts:
    URL = get_url(display,interval,total,(jsonfile['osrs'][-1][0]-15*60*1000)/1000)
...
# Lambda functions
unix_qtr_hr = lambda x: x-divmod(x, 15*60)[1]
